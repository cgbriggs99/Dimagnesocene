...
"""
    ...
    parser.add_argument("--exclude-regex", help = "Exclude a regex from the search.",
                        action = "extend", type = str)
    parser.add_argument("--exlude-file", help = "Exclude all files, patterns, and " +
...
""", default = "psi4 -i #I -o #O", type = str)
    ...
    if args["runner"] == "sequential" :
        ...
        runner.run_parallel(command, in_files, out_files,
                            args["n"] if args["n"] > 0 else None)
    elif args["runner"] == "mpi" :
        runner.run_mpi(command, in_files, out_files,
                       args["n"] if args["n"] > 0 else None)
